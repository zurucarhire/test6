...
export class ApiService {
  ...
  get_products2(name) {
    const url = this.url + "/api/psm/procedure/findallname/"+name;
    return this.http.get<Procedure[]>(url)
      .pipe(map((results) => results));
  }
  ...
  getProceduresById(id: number) {
    const url = this.url + "/api/psm/procedure/findbyid/" + id;
    return this.http.get(url);
  }
  ...
  updateAccount(userid, tag, value) {
    const url = this.url + "/api/psm/user/updateaccount/" + userid + "/" + tag + "/" + value;
    return this.http.put(url, {});
  }

  changePass(userid, oldpassword, newpassword, confirmpassword) {
    const url = this.url + "/api/psm/user/changepassword?userId=" + userid + "&oldPassword=" + oldpassword + "&newPassword=" + newpassword + "&confirmPassword=" + confirmpassword;
    return this.http.put(url, {});
  }
