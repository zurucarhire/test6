...
@Component({
  selector: 'app-account-merchant',
  templateUrl: './account-merchant.component.html',
  styleUrls: ['./account-merchant.component.css']
})
export class AccountMerchantComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.mybusinessname = this.user["user"]["businessName"];
    this.myshoplocation = this.user["user"]["shopLocation"];
    this.myphone = this.user["user"]["phone"];
  ...
  editAccount(tag){
    ...
    } else if (tag == "businessname"){
      value = this.mybusinessname;
      if (this.mybusinessname.length == 0){
        this.notifyService.showError("Please enter business name","Invalid business name");
        return
      }
    } else if (tag == "shoplocation"){
      value = this.myshoplocation;
      if (this.myshoplocation.length == 0){
        this.notifyService.showError("Please enter business name","Invalid shop location");
        return
      }
    } else if (tag == "phone"){
      value = this.myphone;
      if (this.myphone.length != 12){
        this.notifyService.showError("Please enter valid phone number (254*********)","Invalid phone number");
        return
      }
    }
    ...
    this.api.updateAccount(this.user["user"]["userID"], tag, value).subscribe(
      data => {
        ...
        this.mybusinessname = data["businessName"];
        this.myshoplocation = data["shopLocation"];
        this.myphone = data["phone"];
